...
def gng_m7(
    ...
    """Orthogonalized Go/Nogo Task - RW (rew/pun) + noise + bias + pi + 2lr

    Hierarchical Bayesian Modeling of the Orthogonalized Go/Nogo Task 
    using RW (rew/pun) + noise + bias + pi + 2lr [Cavanagh2013]_ with the following parameters:
    "xi" (noise), "win_ep" (win learning rate), "loss_ep" (loss learning rate), "b" (action bias), "pi" (Pavlovian bias), "rhoRew" (reward sensitivity), "rhoPun" (punishment sensitivity).
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('gng_m7').
    ...
    .. code:: python
        ...
        output = gng_m7(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
