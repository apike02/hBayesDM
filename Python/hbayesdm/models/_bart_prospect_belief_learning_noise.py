...
__all__ = ['bart_prospect_belief_learning_noise']


class BartProspectBeliefLearningNoise(TaskModel):
    def __init__(self, **kwargs):
        super().__init__(
            task_name='bart',
            model_name='prospect_belief_learning_noise',
            model_type='',
            data_columns=(
                'subjID',
                'pumps',
...
def bart_prospect_belief_learning_noise(
    ...
    """Balloon Analogue Risk Task - Simple model with a prior belief that is updated IF there is an explosion or the pumps are greater than the prior and decision noise

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task 
    using Simple model with a prior belief that is updated IF there is an explosion or the pumps are greater than the prior and decision noise  with the following parameters:
    "pumps_prior_belief" (prior belief of burst), "learning_rate" (rate at which belief of burst is updated), "inverse_temperature" (governs decision noise, ranging from 0 (stochastic) to infinite (deterministic)).
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('bart_prospect_belief_learning_noise').
    ...
    .. code:: python
        ...
        from hbayesdm.models import bart_prospect_belief_learning_noise

        # Run the model and store results in "output"
        output = bart_prospect_belief_learning_noise(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
        ...
        print_fit(output)
    """
    return BartProspectBeliefLearningNoise(
        data=data,
        niter=niter,
        nwarmup=nwarmup,
        nchain=nchain,
