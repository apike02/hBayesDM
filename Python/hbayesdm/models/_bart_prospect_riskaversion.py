...
__all__ = ['bart_prospect_riskaversion']


class BartProspectRiskaversion(TaskModel):
...
def bart_prospect_riskaversion(
    ...
    """Balloon Analogue Risk Task - Simple model with summative risk aversion only

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task 
    using Simple model with summative risk aversion only  with the following parameters:
    "risk_aversion" (additive value that makes you less likely to pump).
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('bart_prospect_riskaversion').
    ...
    .. code:: python
        ...
        from hbayesdm.models import bart_prospect_riskaversion

        # Run the model and store results in "output"
        output = bart_prospect_riskaversion(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
        ...
        print_fit(output)
    """
    return BartProspectRiskaversion(
        data=data,
        niter=niter,
        nwarmup=nwarmup,
        nchain=nchain,
