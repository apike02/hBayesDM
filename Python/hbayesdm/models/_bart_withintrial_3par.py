...
__all__ = ['bart_withintrial_3par']


class BartWithintrial3Par(TaskModel):
    def __init__(self, **kwargs):
        super().__init__(
            task_name='bart',
            model_name='withintrial_3par',
            model_type='',
            data_columns=(
                'subjID',
                'pumps',
...
def bart_withintrial_3par(
    ...
    """Balloon Analogue Risk Task - BART model where the number of pumps is updated within trials, no learning

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task 
    using BART model where the number of pumps is updated within trials, no learning  with the following parameters:
    "target_pumps" (prior expectation for how many pumps result in explosion), "risk_taking" (risk taking parameter - scales tendency to press more within-trial), "noise" (inverse temperature).
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('bart_withintrial_3par').
    ...
    .. code:: python
        ...
        output = bart_withintrial_3par(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
