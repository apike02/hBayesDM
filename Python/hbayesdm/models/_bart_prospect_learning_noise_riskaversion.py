...
__all__ = ['bart_prospect_learning_noise_riskaversion']


class BartProspectLearningNoiseRiskaversion(TaskModel):
    def __init__(self, **kwargs):
        super().__init__(
            task_name='bart',
            model_name='prospect_learning_noise_riskaversion',
            model_type='',
            data_columns=(
                'subjID',
                'pumps',
...
def bart_prospect_learning_noise_riskaversion(
    ...
    """Balloon Analogue Risk Task - Simple model with a fixed prior belief that is updated IF there is an explosion or the pumps are greater than the prior and decision noise and risk aversion

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task 
    using Simple model with a fixed prior belief that is updated IF there is an explosion or the pumps are greater than the prior and decision noise and risk aversion  with the following parameters:
    "learning_rate" (rate at which belief of burst is updated), "inverse_temperature" (governs decision noise, ranging from 0 (stochastic) to infinite (deterministic)), "risk_aversion" (subtracted from overall expected value).
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('bart_prospect_learning_noise_riskaversion').
    ...
    .. code:: python
        ...
        from hbayesdm.models import bart_prospect_learning_noise_riskaversion

        # Run the model and store results in "output"
        output = bart_prospect_learning_noise_riskaversion(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
        ...
        print_fit(output)
    """
    return BartProspectLearningNoiseRiskaversion(
        data=data,
        niter=niter,
        nwarmup=nwarmup,
        nchain=nchain,
