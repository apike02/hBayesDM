/* BART model: 'Learning rate + belief + noise + risk aversion + Loss aversion'
*  author: Alex Pike
...
transformed parameters {
  ...
  for (i in 1:N){
      ...
      inverse_temperature[i]=exp(mu_pr[3] + sigma[3] * inverse_temperature_pr[i]);
      risk_aversion[i]=exp(mu_pr[4] + sigma[4] * risk_aversion_pr[i]);
      loss_aversion[i]=exp(mu_pr[5] + sigma[5] * loss_aversion_pr[i]);
  }
}
...
generated quantities {
  ...
  real<lower=0> mu_inverse_temperature = exp(mu_pr[3]);
  real<lower=0> mu_risk_aversion = exp(mu_pr[4]);
  real<lower=0> mu_loss_aversion = exp(mu_pr[5]);
