...
transformed parameters {
  ...
  vector<lower=0,upper=1>[N] learningrate;
  vector<lower=0,upper=5>[N] risk_taking;
  vector<lower=0>[N] noise;

  target_pumps = exp(mu_pr[1] + sigma[1] * target_pumps_pr);
  learningrate = Phi_approx(mu_pr[2] + sigma[2] * learningrate_pr);
  risk_taking =  Phi_approx(mu_pr[3] + sigma[3] * risk_taking_pr)*5;
...
model {
  ...
  for (j in 1:N) {
    ...
    for (k in 1:Tsubj[j]) {
      ...
      actual_target_pumps -= learningrate[j]*explosion[j,k]; //updates according to last trial estimate, -1 if explosion
