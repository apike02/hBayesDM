...
transformed parameters {
  ...
  app     = mu_pr[5] + sigma[5] * app_pr;
  av      = mu_pr[6] + sigma[6] * av_pr;
  rhoRew = exp(mu_pr[7] + sigma[7] * rhoRew_pr);
  rhoPun = exp(mu_pr[8] + sigma[8] * rhoPun_pr);
}

model {
// gng_m6: RW(rew/pun) + noise + bias + app + av + 2lr (model in Mkrtchian et al. 2017)
  // hyper parameters
...
generated quantities {
  ...
  mu_app    = mu_pr[5];
  mu_av     = mu_pr[6];
  mu_rhoRew = exp(mu_pr[7]);
  mu_rhoPun = exp(mu_pr[8]);
