#' @templateVar MODEL_FUNCTION bart_prospect_belief_learning_noise
...
#' @templateVar MODEL_NAME Simple model with a prior belief that is updated IF there is an explosion or the pumps are greater than the prior and decision noise
#' @templateVar MODEL_CODE prospect_belief_learning_noise
#' @templateVar MODEL_CITE 
#' @templateVar MODEL_TYPE Hierarchical
#' @templateVar DATA_COLUMNS "subjID", "pumps", "explosion"
#' @templateVar PARAMETERS \code{pumps_prior_belief} (prior belief of burst), \code{learning_rate} (rate at which belief of burst is updated), \code{inverse_temperature} (governs decision noise, ranging from 0 (stochastic) to infinite (deterministic))
...
bart_prospect_belief_learning_noise <- hBayesDM_model(
  task_name       = "bart",
