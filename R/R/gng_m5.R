#' @templateVar MODEL_FUNCTION gng_m5
#' @templateVar CONTRIBUTOR 
#' @templateVar TASK_NAME Orthogonalized Go/Nogo Task
#' @templateVar TASK_CODE gng
#' @templateVar TASK_CITE 
#' @templateVar MODEL_NAME RW (rew/pun) + noise + appbias + avbias + bias
#' @templateVar MODEL_CODE m5
#' @templateVar MODEL_CITE (Cavanagh et al., 2013)
#' @templateVar MODEL_TYPE Hierarchical
#' @templateVar DATA_COLUMNS "subjID", "cue", "keyPressed", "outcome"
#' @templateVar PARAMETERS \code{xi} (noise), \code{ep} (learning rate), \code{b} (action bias), \code{app} (Pavlovian approach bias), \code{av} (Pavlovian avoid bias), \code{rhoRew} (reward sensitivity), \code{rhoPun} (punishment sensitivity)
